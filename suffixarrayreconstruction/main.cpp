...
void testCase() {
    ...
    for (auto& [p, s] : ambig) {
        int str = s.find('*');
        string a = s.substr(0, str);
        string b = s.substr(str+1, s.length());
        for (int j = p; j < p + a.length(); ++j) {
            if (res[j] == '0') {
                res[j] = a[j-p];
            } else if (res[j] != a[j-p]) {
                cout << "IMPOSSIBLE" << endl;
