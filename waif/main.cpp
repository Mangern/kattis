...
void testCase() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = 0; j < k; ++j) {
            int t;
            cin >> t, --t;
            mf.add_edge(1+i, 1+n+t, 1);
        }
    }

    vector<bool> incat(m, 0);
    for (int i = 0; i < p; ++i) {
        int k;
        cin >> k;
        for (int j = 0; j < k; ++j) {
            int t;
            cin >> t, --t;
            mf.add_edge(1+n+t, 1+n+m+i, 1);
            incat[t] = 1;
        }
        int r;
        cin >> r;
        mf.add_edge(1+n+m+i, 1+n+m+p, r);
    }
    for (int t = 0; t < m; ++t) {
        if (!incat[t]) {
            mf.add_edge(1+n+t, 1+n+m+p, 1);
        }
    }
    for (int i = 0; i < n; ++i) {
        mf.add_edge(0, 1+i, 1);
    ...
    cout << mf.max_flow(0, 1+n+m+p) << endl;
}
