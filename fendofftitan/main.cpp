...
void testCase() {
    ...
    vector<vector<tup>> adj(n, vector<tup>());
    ...
    vector<tup> dist(n, {INFLL, INFLL, INFLL});

    priority_queue<pair<tup, int>, vector<pair<tup, int>>, greater<pair<tup, int>>> pq;
    dist[s] = {0, 0, 0};
    pq.push({{0,0,0}, s});
    ...
    cout << dist[t][2] << " " << dist[t][1] << " " << dist[t][0] << endl;
}
