...
void testCase() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        lead[0] = max(lead[0], cnt[0] - cnt[1]);
        lead[1] = max(lead[1], cnt[1] - cnt[0]);

        strk[sc[i]] = max(strk[sc[i]], i - prv[1 - sc[i]]);

        prv[sc[i]] = i;
    }

    if (lead[0] > lead[1] && strk[0] > strk[1]
     || lead[1] > lead[0] && strk[1] > strk[0]
     || lead[0] == lead[1] && strk[0] == strk[1]) {
        cout << "Agree" << endl;
    } else {
        cout << "Disagree" << endl;
    }
