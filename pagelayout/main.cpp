...
int solve(int i, int cur = 0) {
    if (i == n) {
        return cur;
    }
    for (auto& r : use) {
        if (inter(r, recs[i])) return solve(i+1, cur);
    }
    int r = cur;
    use.push_back(recs[i]);
    r = max(r, solve(i+1, cur + recs[i].w * recs[i].h));
    use.pop_back();
    r = max(r, solve(i+1, cur));
    return r;
}
...
void testCase() {
    ...
    for (int i = 0; i < n; ++i) {
        cin >> recs[i].w >> recs[i].h >> recs[i].x >> recs[i].y;
    }
