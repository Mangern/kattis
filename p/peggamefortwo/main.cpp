...
ll solve(const state &s, bool turn) {
    if (dp.count(s)) return dp[s];

    // for each move:
    //   ret = max ret, value(move) - solve(s ^ move)
    ll ret = 0;
    bool seen = 0;
    for (int i = 0; i < 5; ++i) {
        for (int j = 0; j <= i; ++j) {
            if (!s[toidx(i, j)]) continue;
            for (int dir = 0; dir < 6; ++dir) {
                ...
                if (s[toidx(ni, nj)]) continue;
                if (!s[toidx(pi, pj)]) continue;
                ll score = s[toidx(pi, pj)] * s[toidx(i, j)];
                state cpy = s;
                cpy[toidx(ni, nj)] = s[toidx(pi, pj)];
                cpy[toidx(pi, pj)] = 0;
                cpy[toidx(i, j)] = 0;

                ll other = solve(cpy, !turn);
                if (turn) {
                    if (seen) {
                        ret = min(ret, other - score);
                    } else {
                        ret = other - score;
                        seen = 1;
                    }
