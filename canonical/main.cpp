...
inline ll count_dp(const vll& sorted, ll target) {
    ...
    for (int i = sorted.size() - 1; i >= 0; --i) {
        ans = min(ans, 1 + count_dp(sorted, target - sorted[i]));
    }
    return ans;
}
...
void testCase() {
    ...
    dpgreedy[0] = 0;
    int ptr = 0;
    for (ll i = 1; i <= a[n-1] + a[n-2]; ++i) {
        while (ptr < n - 1 && a[ptr+1] <= i)++ptr;
        dpgreedy[i] = dpgreedy[i - a[ptr]] + 1;
    }

    memset(dp, -1, sizeof dp);
    for (ll target = a[n-1] + a[n-2]; target >= a[n-2]; --target) {
        ll greedy = dpgreedy[target];
        ll dp = count_dp(a, target);

        if (greedy != dp) {
            //cout << "failed for target: " << target << endl;
            //cout << "DP:" << dp << ", Greedy:" << greedy;
