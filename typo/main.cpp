...
struct MultiHash {
    Hash<> ha;
    Hash<311, (ll)1e9+7> hb;

    MultiHash(string s) : ha(s), hb(s) { }

    ll hash(int l, int r) {
        unsigned long long a = ha.hash(l, r);
        unsigned long long b = hb.hash(l, r);
        return (a << 32) | b;
    }

    ll hrem(int i) {
        unsigned long long a = ha.hrem(i);
        unsigned long long b = hb.hrem(i);
        return (a << 32) | b;
    }
};
const int mxN = 2e5+6;
...
void testCase() {
    ...
    for (int i = 0; i < n; ++i) {
        cin >> a[i];
        hs.emplace_back(a[i]);
        hashes.insert(hs[i].hash(0, a[i].length()));
    }

    bool found = 0;
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < a[i].length(); ++j) {
            if (hashes.count(hs[i].hrem(j))) {
                found = 1;
                cout << a[i] << endl;
                break;
            }
