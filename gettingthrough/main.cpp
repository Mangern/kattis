...
struct UnionFind {
    ...
    bool blocks(){
        return std::find(all(flag), 3) != end(flag);
    }
...
void testCase() {
    ...
    while (hi - lo > EPS) {
        ...
        for (int i = 0; i < n; ++i) {
            if (w - (xs[i] + rs[i]) < 2 * r)uf.flag[i] |= 1;
            if (xs[i] - rs[i] < 2 * r)uf.flag[i] |= 2;
        }

        for (int i = 0; i < n; ++i) {
            for (int j = i + 1; j < n; ++j) {
                double dx = (xs[i] - xs[j]) * (xs[i] - xs[j]);
                double dy = (ys[i] - ys[j]) * (ys[i] - ys[j]);

                if (dx + dy <= (2*r + rs[i] + rs[j]) * (2 * r + rs[i] + rs[j])) {
                    uf.join(i, j);
                }
            }
        }
