...
struct AhoCorasick {
	enum {alpha = 27, first = 'a'}; // change this!
	struct Node {
		// (nmatches is optional)
		int back = -1, next[alpha], start = -1, end = -1, nmatches = 0, depth = 0;
        ll val = 0;
	...
	AhoCorasick(vector<string>& pat, vii riv) : N(1, -1) {
        pos.resize(pat.size());
        ...
        for (int i = 0; i < riv.size(); ++i) {
            auto [u,v] = riv[i];
            N[pos[u]].val = (N[pos[u]].val + B - pw[i]) % B;
            N[pos[v]].val = (N[pos[v]].val + pw[i]) % B;
        }
		...
		for (q.push(0); !q.empty(); q.pop()) {
            ...
            for (int i = 0; i < alpha; ++i) {
				...
				if (ed == -1) ed = y;
                    ...
                    N[ed].depth = N[n].depth + 1;

                    N[ed].val = (N[ed].val + N[y].val) % B;
...
void testCase() {
    ...
    AhoCorasick aho(uni, riv);

    while (getline(cin, line)) {
        for (int i = 0; i < line.length(); ++i) {
            if (line[i] == ' ')line[i] = (char)('z'+1);

        }
        ll val = 0;
        aho.find(line, [&] (int u) {
            val = (val + aho.N[u].val) % B;
        });
