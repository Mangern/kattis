...
int ptr = 0;
int iptr;
void create(const string &s, vector<vi>& adj, int u = 0) {
    if (iptr == 0) ptr = 1;
...
string srep(const vector<vi>& adj, int u = 0) {
    vector<string> creps;
    for (int v : adj[u]) {
        creps.push_back(srep(adj, v));
    }
    sort(all(creps), [](const auto &a, const auto &b) {
...
void testCase() {
    ...
    vector<vi> sadj(n), tadj(n);
    iptr = 0;
    create(s, sadj);
    iptr = 0;
    create(t, tadj);

    cout << (srep(sadj) == srep(tadj) ? "same" : "different") << endl;
}
