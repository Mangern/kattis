...
FenwickTree<ll> ft_lo;
FenwickTree<ll> ft_hi;
...
void testCase() {
    ...
    for (int i = 0; i < n; ++i) {
        if (col[i] < 50) {
            ft_lo.add(fst[i], 1LL<<col[i]);
        } else {
            ft_hi.add(fst[i], 1LL<<(col[i]-50));
        }
    }
    ...
    while (q-->0) {
        ...
        if (!k) {
            ll s_lo = ft_lo.query(fst[x], lst[x] - 1);
            ll s_hi = ft_hi.query(fst[x], lst[x] - 1);
            cout << __builtin_popcountll(s_lo) + __builtin_popcountll(s_hi) << endl;
        } else {
            --k;
            int cur = col[x];
            if (cur == k) continue;
            ...
            if (cur < 50) {
                ft_lo.add(fst[x], 1LL<<cur);
            } else {
                ft_hi.add(fst[x], 1LL<<(cur-50));
            }
            if (k < 50) {
                ft_lo.add(fst[x], 1LL<<k);
            } else {
                ft_hi.add(fst[x], 1LL<<(k-50));
            }
            col[x] = k;
        }
    }
