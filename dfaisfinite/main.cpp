...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        for (char c : dfa.alphabet) {
            adj[i].insert(dfa.trans[i][c]);
            radj[dfa.trans[i][c]].insert(i);
        }
    }
    ...
    function<bool(int,int,int)> dfs_cyc = [&](int u, int cf, int t) {
        vis[u] = t;
        for (int v : radj[u]) if (v != cf && reach[v]) {
            if (vis[v] == vis[u]) return true;
            else if (!vis[v]) {
                if (dfs_cyc(v, u, t)) return true;
            }
        }
        return false;
    };
    ...
    int t = 1;
    for (int u = 0; u < n; ++u) if (reach[u] && dfa.final[u] && !vis[u]) {
        if (dfs_cyc(u, -1, t)) {
            cout << "infinite" << endl;
            return 0;
        }
