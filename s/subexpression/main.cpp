...
void convert(int l, int r) {
    ...
    node[sub] = to_string(timer++);
    int lpar = 0;
    for (lpar = l; lpar < r; ++lpar) {
        if (s[lpar] == '(') break;
    }
    ...
    result += s_view.substr(l, lpar + 1 - l);
    if (s[j] == '(')j = paren_match[j] + 1;
    assert(s[j] == ',');
    convert(lpar + 1, j);
    result += ",";
    convert(j + 1, r - 1);
