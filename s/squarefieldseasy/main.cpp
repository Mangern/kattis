...
int n, k;
int dp[1<<7];
int mincover(int mask, ll len, const vii &pts) {
    if (mask == 0) return 0;
    ...
    ret = 10000;
    FORMASK(i, mask) {
        ret = min(ret, 1 + mincover(mask ^ (1<<i), len, pts));
        FORMASK(j, mask) if (j != i) {
            if (pts[i][0] <= pts[j][0] && pts[j][0] <= pts[i][0] + len && pts[i][1] - len <= pts[j][1] && pts[j][1] <= pts[i][1]) {
                // left edge: i, bottom edge: j
                int nmask = mask;

                FORMASK(k, mask) {
                    if (pts[i][0] <= pts[k][0] && pts[k][0] <= pts[i][0] + len && pts[j][1] <= pts[k][1] && pts[k][1] <= pts[j][1] + len) {
                        nmask ^= (1<<k);
                    }
                }
                ret = min(ret, 1 + mincover(nmask, len, pts));
            }

            if (pts[i][0] <= pts[j][0] && pts[j][0] <= pts[i][0] + len && pts[i][1] <= pts[j][1] && pts[j][1] <= pts[i][1] + len) {
                // left edge: i, top edge: j
                int nmask = mask;
                FORMASK(k, mask) {
                    if (pts[i][0] <= pts[k][0] && pts[k][0] <= pts[i][0] + len && pts[j][1] - len <= pts[k][1] && pts[k][1] <= pts[j][1]) {
                        nmask ^= (1<<k);
                    }
                }
            ...
            if (pts[i][0] - len <= pts[j][0] && pts[j][0] <= pts[i][0] && pts[i][1] - len <= pts[j][1] && pts[j][1] <= pts[i][1]) {
                // right edge: i, bottom edge: j
                int nmask = mask;

                FORMASK(k, mask) {
                    if (pts[i][0] - len <= pts[k][0] && pts[k][0] <= pts[i][0] && pts[j][1] <= pts[k][1] && pts[k][1] <= pts[j][1] + len) {
                        nmask ^= (1<<k);
                    }
                }
                ret = min(ret, 1 + mincover(nmask, len, pts));
            ...
            if (pts[i][0] - len <= pts[j][0] && pts[j][0] <= pts[i][0] && pts[i][1] <= pts[j][1] && pts[j][1] <= pts[i][1] + len) {
                // right edge: i, top edge: j
                int nmask = mask;

                FORMASK(k, mask) {
                    if (pts[i][0] - len <= pts[k][0] && pts[k][0] <= pts[i][0] && pts[j][1] - len <= pts[k][1] && pts[k][1] <= pts[j][1]) {
                        nmask ^= (1<<k);
                    }
                }
                ret = min(ret, 1 + mincover(nmask, len, pts));
...
bool check(ll len, const vii &pts) {
    memset(dp, -1, sizeof dp);
    return mincover((1<<n)-1, len, pts) <= k;
}
...
void testCase() {
    ...
    for (auto &pp : pts) {
        cin >> pp[0] >> pp[1];
        mn = min(mn, min(pp[0], pp[1]));
        mx = max(mx, max(pp[0], pp[1]));
    }
