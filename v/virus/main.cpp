...
void testCase() {
    ...
    int pref, suf;
    for (pref = 0; pref < min(s.length(), t.length()); ++pref)
        if (s[pref] != t[pref]) break;

    for (suf = -1; (s.length() + suf >= 0) && (t.length() + suf >= 0); --suf) {
        if (s[s.length() + suf] != t[t.length() + suf])break;
    }

    int rem = min((int)s.length(), -suf - 1 + pref);

    cout << max(0, (int)t.length() - rem) << endl;
}
