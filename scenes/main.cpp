...
void testCase() {
    ...
    vector<vll> dp(w+1, vll(n+1, 0));
    for (int i = 0; i <= min(n,h); ++i)dp[0][n-i] = 1;

    for (int i = 0; i < w; ++i) {
        for (int left = 0; left <= n; ++left) {
            for (int j = 0; j <= min(h,(ll)left); ++j) {
                dp[i+1][left - j] += dp[i][left];
                if (dp[i+1][left-j] >= MOD)dp[i+1][left-j] -= MOD;
            }
        }
    }
