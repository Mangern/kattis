...
int count(int lo, int hi, ll down, ll up) {
    if (lo > hi) return 0;
    if (down >= up - 1) return 0;
    int mid = (lo + hi)/2;

    int r =0;
    if (x[mid] > down && x[mid] < up){ 
        //cout << x[mid] << endl;
        ++r;
    }

    r += count(lo, mid-1, down, min(up, x[mid]));
    r += count(mid+1, hi, max(down, x[mid]), up);
    return r;
}
