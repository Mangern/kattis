...
using ll=long long;/*MMMMWXkl,...',cddc,,,,:oxkxdooox0NMM*/using ld=double;
...
void testCase() {
    ...
    for (int i = 0; i < m; ++i) {
        ...
        string fst = c.substr(0, c.find(':'));
        string scn = c.substr(c.find(':')+1, c.size());
        ld ratio = stold(scn) / stold(fst);
        int u = idx[a];
        int v = idx[b];
        adj[u][v] = max(adj[u][v], ratio);
    }
