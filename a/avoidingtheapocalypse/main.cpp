...
struct MaxFlow {
    ...
    ll dfs(int u, int t, ll f = MaxFlow::INF) {
        if (u == t || f == 0) return f;
...
void testCase() {
    ...
    for (int i = 0; i < m; ++i) {
        ...
        for (int j = 0; j < max_time; ++j) {
            if (j + t >= max_time) continue;
            // Traversing edge (u, j) -> (v, j + t). Max p people.
            mf.add_edge(j * n + u, (j + t) * n + v, p);
        }
    }
    ...
    for (int u = 0; u < n; ++u) {
        for (int j = 0; j < max_time; ++j) {
            if (j < max_time - 1)
                // Stay on the same spot for one time step
                mf.add_edge(j * n + u, (j + 1) * n + u, MaxFlow::INF);

        }
        if (goals[u]) {
            // Reached facility
            mf.add_edge((max_time - 1) * n + u, t, MaxFlow::INF);
        }
    }
    // Could also do min(max_flow, g)
    mf.add_edge(s, is, g);
