...
void testCase() {
    ...
    vector<vi> dp(n, vi(1<<(n-1), -1));

    function<int(int, int)> solve = [&] (int u, int mask) {
        if (mask == 0) {
            return (int)!forb.count({u, 0});
        }
        int& ret = dp[u][mask];
        if (ret != -1)return ret;
    ...
    cout << (solve(0, (1<<(n-1))-1) * 4951) % MOD << endl;
}
