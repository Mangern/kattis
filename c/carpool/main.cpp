...
// start at u, visit everything in mask and end at n+1
ll n, m;
ll dp[mxN][1<<mxN];
ll dist[mxN][mxN];
...
ll tsp(int u, int mask) {
    ll &r = dp[u][mask];
    if (r != -1) return r;
    if (mask == 0)return r = dist[u][n + 1];

    r = INFLL;
    FORMASK(v, mask) {
        r = min(r, dist[u][v] + tsp(v, mask ^ (1<<v)) + 5);
    }
    return r;
}
...
void testCase() {
    ...
    for (int ss = MSK; ss; ss = (ss - 1) & MSK) {
        if (__builtin_popcount(ss) > 5) continue;
        int rem = (MSK ^ ss);
        ...
        if (rem) {
            ll resttime = INFLL;
            for (int ss2 = rem; nc > 1 && ss2; ss2 = (ss2 - 1) & rem) {
                if (__builtin_popcount(ss2) > 5) continue;
                if (__builtin_popcount(rem ^ ss2) > 5) continue;
                ...
                if (rem2) {
                    if (nc > 2)
                        cur = max(cur, tsp(0, rem2));
                    else cur = INFLL;
                }

                resttime = min(resttime, cur);
            }

            grptime = max(grptime, resttime);
        } 
        ans = min(ans, grptime);
    }
    cout << ans << endl;
