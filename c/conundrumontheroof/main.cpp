...
void testCase() {
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; ++j) {
            for (auto [ni, nj] : vii{{i-1,j},{i,j-1},{i+1,j},{i,j+1}}) {
                if (ni < 0 || n <= ni || nj < 0 || m <= nj) continue;
                int u = i * m + j;
                int v = ni * m + nj;
                if (grid[ni][nj] >= grid[i][j]) {
                    if (grid[ni][nj] - grid[i][j] <= du) {
                        adj[u].push_back(v);
                    }
                } else {
                    if (grid[i][j] - grid[ni][nj] <= dn) {
                        adj[u].push_back(v);
                    }
                }
            }
    ...
    if (!vis[dst]) {
        cout << "Nu er Eyleifur i bobba!" << endl;
        return;
    }
    vis.assign(n * m, 0);
    vis[dst] = 1;
