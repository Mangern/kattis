...
int main() {
    ...
    int maxl = (500000 + n - 1) / n + 1;

    vector<vector<int>> adj(n);

    for (int i = 0; i < n; ++i) {
        for (char c : dfa.alphabet) {
            adj[i].push_back(dfa.trans[i][c]);
        }
    }

    // dp[i][j]: how many paths of length j to a final state from i?
    vector<vector<ll>> dp(n, vector<ll>(maxl, -1));

    vector<int> vis(n, 0);

    function<ll(int,int)> dfs = [&](int u, int l) {
        if (dp[u][l] != -1) {
            return dp[u][l];
        }
        if (dfa.final[u] && l == 0) {
