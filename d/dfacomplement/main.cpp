...
DFA dfa_complement(const DFA& dfa) {
    DFA new_dfa(dfa.alphabet);
    new_dfa.num_states = dfa.num_states;
    new_dfa.trans = dfa.trans;
    new_dfa.initial = dfa.initial;
    new_dfa.final = vector<bool>(dfa.num_states, 0);
    for (int i = 0; i < dfa.num_states; ++i) {
        if (!dfa.final[i])new_dfa.mark_final(i);
    }
    return new_dfa;
}
