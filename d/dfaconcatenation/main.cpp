...
struct NFA {
    ...
    vector<int> eps_closure(vector<int> subset) {
        set<int> have(subset.begin(), subset.end());
        for (int i = 0; i < subset.size(); ++i) {
            int u = subset[i];
            if (trans[u].count(EPS)) {
                for (int v : trans[u][EPS]) {
                    if (!have.count(v)) {
                        have.insert(v);
                        subset.push_back(v);
                    }
...
DFA dfa_concat(const DFA& dfa1, const DFA& dfa2) {
    ...
    // now subset construction to make the DFA
    DFA new_dfa(new_alpha);
