...
bool can(ll maxc) {
    ...
    for (int i = 0; i < l; ++i) {
        ...
        mf.add_edge(i, a + l, 1);
        mf.add_edge(i, b + n + l, 1);
    }

    for (int i = 0; i < n; ++i) {
        mf.add_edge(i + l, t, maxc);
    }
    for (int i = 0; i < m; ++i) {
        mf.add_edge(i + n + l, t, maxc);
    }
    return mf.max_flow(s, t) == l;
}
...
void testCase() {
    ...
    for (int i = 0; i < l; ++i) {
        ...
        for (auto idx : mf.adj[i]) {
            ...
            if (cap - flow == 0 && l <= v && v < l + n + m) {
                win = v;
            }
        }

        if (win < l + n) {
            cout << "INDV " << win - l + 1 << endl;
        } else {
            cout << "CORP " << win - l - n + 1 << endl;
        }
    }
}
