...
void testCase() {
    ...
    for (int i = 0; i < n; ++i) {
        has[uf.find(i)].push_back(i);
    }

    ll ans = 0;
    for (auto &[p, idx] : has) {
        vector<int> idx_w = idx;
        vector<int> idx_v = idx;
        sort(all(idx_w), [&](auto i, auto j) { return w[i] < w[j]; });
        sort(all(idx_v), [&](auto i, auto j) { return v[i] < v[j]; });

        for (int i = 0; i < idx_w.size(); ++i) {
            ans += w[idx_w[i]] * v[idx_v[i]];
        }
    }
    cout << ans << endl;
}
