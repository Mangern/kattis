...
void testCase() {
    ...
    for (int i = 0; i < mxN; ++i) if (i != 1009) {
        if (id[i] != -1) {
            if (i < 2000) {
                mf.add_edge(s, id[i], 1);
            } else {
                mf.add_edge(id[i], t, 1);
            }
        }
    }

    int k = mf.max_flow(s, t);

    vector<bool> vis(n, 0);
    queue<int> q;
    for (int i = 1000; i < 2000; ++i) if (id[i] != -1) {
        if (mf.dist[id[i]] == mf.dist[s] + 1) {
            q.push(id[i]);
            vis[id[i]] = 1;
        }
    }
    ...
    set<int> tl, tr;
    for (int i = 0; i < n; ++i) if (rid[i] < 2000)tl.insert(i);
    ...
    while (q.size()) {
        ...
        tl.erase(u);
        if (rid[u] >= 2000)tr.insert(u);

        for (auto idx : mf.adj[u]) {
            auto [v, cap, flow] = mf.edges[idx];
            if (cap - flow > 0 && !vis[v]) {
                vis[v] = 1;
                q.push(v);
            }
