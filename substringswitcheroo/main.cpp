...
using cnt = array<int, 26>;

vector<cnt> count(const string& s) {
    vector<cnt> result(s.length() + 1);
    for (int i = 0; i < 26; ++i)result[0][i] = 0;
    for (int i = 0; i < s.length(); ++i) {
        for (int j = 0; j < 26; ++j) {
            result[i+1][j] = result[i][j] + (j == (s[i]-'a'));
        }
    }
    return result;
}
...
void testCase() {
    ...
    if (ans == ii{-1, -1}) {
        ...
        cout << a.substr(ans[0], ans[1] - ans[0]) << endl;
    }
}
