...
void testCase() {
    ...
    vector<pair<point, double>> stamps(n);
    for (int i = 0; i < n; ++i) {
        cin >> stamps[i].first.x >> stamps[i].first.y >> stamps[i].second;
    }
    double tot = 0.0;
    point prev = stamps[0].first;
    double real = 0.0;
    ...
    for (int i = 1; i < n; ++i)real += dist(stamps[i].first, stamps[i-1].first);

    while (g < stamps[n-1].second) {
        while (stamps[ptr].second <= g)++ptr;
        double dt = g - stamps[ptr-1].second;
        double ttime = stamps[ptr].second - stamps[ptr-1].second;
        double speed = dist(stamps[ptr].first, stamps[ptr-1].first) / ttime;
        point p = (p2vec({0,0},stamps[ptr-1].first) + p2vec(stamps[ptr-1].first, stamps[ptr].first) * (dt / ttime)).as_point();
        tot += dist(p, prev);
        prev = p;
        g += t;
    }

    tot += dist(stamps[n-1].first, prev);
    //cout << real << endl;
    //cout << tot << endl;
    cout << setprecision(20) << (real - tot) * 100 / real << endl;
}
