...
void testCase() {
    ...
    for (int i = 0; i < n + m; ++i) {
        ...
        ll start = stoll(s.substr(0, 2)) * 60 + stoll(s.substr(3));
        ll end = stoll(t.substr(0, 2)) * 60 + stoll(t.substr(3));

        orders.push_back({start, end, i >= n});
    }
    ...
    priority_queue<ll, vll, greater<ll>> ata, atb;

    int ansa = 0;
    int ansb = 0;

    for (auto [s, t, isBtoA] : orders) {
        if (isBtoA) {
            if (atb.size() && atb.top() <= s) {
                atb.pop();
            } else ++ansb;
            ata.push(t + turn);
        } else {
            if (ata.size() && ata.top() <= s) {
                ata.pop();
            } else ++ansa;
            atb.push(t + turn);
        }
