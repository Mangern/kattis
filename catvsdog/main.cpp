...
void dfs(int u, int c, vector<vi> &adj, vi &col) {
    if (col[u] != 0) {
        assert(col[u] == c);
        return;
    }
...
void testCase() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        pref[i][0] = stoll(s.substr(1));
        pref[i][1] = stoll(t.substr(1));
        if (s[0] == 'C')pref[i][0] *= -1;
        if (t[0] == 'C')pref[i][1] *= -1;
    }
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = i + 1; j < n; ++j) {
            if (pref[i][0] == pref[j][1] || pref[i][1] == pref[j][0]) {
                adj[i].push_back(j);
                adj[j].push_back(i);
            }
        }
    ...
    for (int i = 0; i < n; ++i) if (col[i] == 2) {
        mf.add_edge(i, t, 1);
    }

    cout << n - mf.max_flow(s, t) << endl;
}
